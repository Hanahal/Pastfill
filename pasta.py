...
def generate_template_csv() -> bytes:
    ...
    for r in [(75.0, 100.0), (50.0, 98.5), (37.5, 96.0), (25.0, 90.0),
              (19.0, 82.0), (9.5, 65.0), (4.75, 48.0), (2.00, 35.0),
              (0.85, 25.0), (0.425, 18.0), (0.25, 13.0), (0.15, 9.0),
              (0.106, 6.5), (0.075, 5.2)]:
        out.write(f"{r[0]},{r[1]}\n")
...
# ─────────────────────────────────────────────
#  TAMICES NORMALIZADOS ASTM D6913
# ─────────────────────────────────────────────
TAMICES_ASTM = [75.0, 50.0, 37.5, 25.0, 19.0, 9.5, 4.75,
                2.00, 0.850, 0.425, 0.250, 0.150, 0.106, 0.075]

TAMICES_LABELS = {
    75.0: '3"', 50.0: '2"', 37.5: '1½"', 25.0: '1"',
    19.0: '¾"', 9.5: '⅜"', 4.75: 'N°4', 2.00: 'N°10',
    0.850: 'N°20', 0.425: 'N°40', 0.250: 'N°60',
    0.150: 'N°100', 0.106: 'N°140', 0.075: 'N°200'
}

# ─────────────────────────────────────────────
#  CLASIFICACION SUCS (ASTM D2487)
# ─────────────────────────────────────────────
def clasificar_sucs(pct_grava, pct_arena, pct_finos, Cu, Cc, LL=None, IP=None):
    """
    Clasificación SUCS simplificada según ASTM D2487.
    Retorna (símbolo, nombre).
    """
    if pct_finos >= 50:
        # Suelo fino
        if LL is None or IP is None:
            return "ML/CL", "Limo / Arcilla (datos de plasticidad no disponibles)"
        if LL < 50:
            if IP >= 4 and IP >= 0.73 * (LL - 20):
                if IP <= 7:
                    return "CL-ML", "Arcilla limosa"
                return "CL", "Arcilla magra"
            else:
                return "ML", "Limo"
        else:
            if IP >= 0.73 * (LL - 20):
                return "CH", "Arcilla grasa"
            else:
                return "MH", "Limo elástico"
    else:
        # Suelo grueso
        if pct_grava > pct_arena:
            # GRAVA
            if pct_finos < 5:
                if Cu >= 4 and 1 <= Cc <= 3:
                    return "GW", "Grava bien gradada"
                else:
                    return "GP", "Grava mal gradada"
            elif pct_finos <= 12:
                if Cu >= 4 and 1 <= Cc <= 3:
                    return "GW-GM", "Grava bien gradada con limo"
                else:
                    return "GP-GM", "Grava mal gradada con limo"
            else:
                if IP is not None and IP >= 7 and (IP >= 0.73 * ((LL or 0) - 20)):
            ...
            if pct_finos < 5:
                if Cu >= 6 and 1 <= Cc <= 3:
                    ...
                    return "SP", "Arena mal gradada"
            elif pct_finos <= 12:
                if Cu >= 6 and 1 <= Cc <= 3:
                    return "SW-SM", "Arena bien gradada con limo"
                else:
                    return "SP-SM", "Arena mal gradada con limo"
            else:
                if IP is not None and IP >= 7 and (IP >= 0.73 * ((LL or 0) - 20)):
...
def calcular_granulometria(df_granu):
    """
    A partir de DataFrame con columnas Tamiz_mm y Pasa_pct,
    calcula porcentajes retenidos, acumulados, D10/D30/D60, Cu, Cc
    y fracciones grava/arena/finos.
    """
    df = df_granu.copy().sort_values("Tamiz_mm", ascending=False).reset_index(drop=True)
    df["Pasa_pct"] = pd.to_numeric(df["Pasa_pct"], errors="coerce").clip(0, 100)
    df["Tamiz_mm"] = pd.to_numeric(df["Tamiz_mm"], errors="coerce")
    df = df.dropna()

    # Etiquetas
    df["Etiqueta"] = df["Tamiz_mm"].map(lambda x: TAMICES_LABELS.get(x, f"{x} mm"))

    # Retenido acumulado y retenido individual
    df["Ret_acum_pct"] = 100 - df["Pasa_pct"]
    df["Ret_indiv_pct"] = df["Ret_acum_pct"].diff().fillna(df["Ret_acum_pct"])
    df.loc[df.index == 0, "Ret_indiv_pct"] = df.loc[0, "Ret_acum_pct"]

    # Fracciones (ASTM D2487)
    # Grava: retenido en N°4 (4.75 mm)
    # Arena: pasa N°4, retenido en N°200 (0.075 mm)
    # Finos: pasa N°200

    pasa_N4 = float(np.interp(4.75, df["Tamiz_mm"].values[::-1], df["Pasa_pct"].values[::-1]))
    pasa_N200 = float(np.interp(0.075, df["Tamiz_mm"].values[::-1], df["Pasa_pct"].values[::-1]))

    pct_grava = 100 - pasa_N4
    pct_arena = pasa_N4 - pasa_N200
    pct_finos = pasa_N200

    # D10, D30, D60 por interpolación logarítmica
    tamices_asc = df["Tamiz_mm"].values[::-1]
    pasa_asc = df["Pasa_pct"].values[::-1]

    def get_D(pct_target):
        if pct_target < pasa_asc[0] or pct_target > pasa_asc[-1]:
            return None
        return float(np.interp(pct_target, pasa_asc, tamices_asc))
    ...
    return df, pct_grava, pct_arena, pct_finos, D10, D30, D60, Cu, Cc, pasa_N4, pasa_N200

# ─────────────────────────────────────────────
#  EVALUACION GRANULOMETRIA PARA PASTA (ASTM D1140 + criterio PF)
# ─────────────────────────────────────────────
def evaluar_granulometria_pasta(pct_finos, pasa_N200, rho_relave, contenido_solidos_pct):
    """
    Criterios de aptitud del relave para Relleno en Pasta:
    - % finos (< 0.075 mm) >= 15% (criterio Paste Fill)
    - Densidad de sólidos coherente
    - Contenido de sólidos en pasta vs granulometría
    """
    resultados = []

    # Criterio 1: Pasta Fill requiere >= 15% de finos (ASTM D1140 / criterio minero)
    ok1 = pct_finos >= 15
    resultados.append({
        "Criterio": "% finos (< 0.075 mm) ≥ 15%",
        "Valor": f"{pct_finos:.1f}%",
        "Límite": "≥ 15%",
        "Estado": "✅ CUMPLE" if ok1 else "❌ NO CUMPLE"
    })

    # Criterio 2: % finos <= 70% (evitar segregación extrema)
    ok2 = pct_finos <= 70
    resultados.append({
        "Criterio": "% finos (< 0.075 mm) ≤ 70%",
        "Valor": f"{pct_finos:.1f}%",
        "Límite": "≤ 70%",
        "Estado": "✅ CUMPLE" if ok2 else "❌ NO CUMPLE"
    })

    # Criterio 3: Pasa N°200 ≥ 15% (confirmación por lavado ASTM D1140)
    ok3 = pasa_N200 >= 15
    resultados.append({
        "Criterio": "Material pasa N°200 (lavado) ≥ 15%",
        "Valor": f"{pasa_N200:.1f}%",
        "Límite": "≥ 15%",
        "Estado": "✅ CUMPLE" if ok3 else "❌ NO CUMPLE"
    })

    # Criterio 4: Densidad relave coherente (2600–3200 kg/m³)
    ok4 = 2600 <= rho_relave <= 3200
    resultados.append({
        "Criterio": "Densidad relave (2600–3200 kg/m³)",
        "Valor": f"{rho_relave:.0f} kg/m³",
        "Límite": "2600–3200",
        "Estado": "✅ CUMPLE" if ok4 else "⚠️ REVISAR"
    })

    # Criterio 5: Contenido sólidos pasta (70–85%)
    ok5 = 70 <= contenido_solidos_pct <= 85
    resultados.append({
        "Criterio": "Contenido sólidos en pasta (70–85%)",
        "Valor": f"{contenido_solidos_pct:.1f}%",
        "Límite": "70–85%",
        "Estado": "✅ CUMPLE" if ok5 else "⚠️ REVISAR"
    })

    n_cumple = sum([ok1, ok2, ok3, ok4, ok5])
    apto = n_cumple >= 4

    return pd.DataFrame(resultados), n_cumple, apto
...
# Datos granulométricos por defecto (relave típico para Paste Fill)
DEFAULT_GRANU = pd.DataFrame({
    "Tamiz_mm": [75.0, 50.0, 37.5, 25.0, 19.0, 9.5, 4.75,
                 2.00, 0.850, 0.425, 0.250, 0.150, 0.106, 0.075],
    "Pasa_pct": [100.0, 98.5, 96.0, 90.0, 82.0, 65.0, 48.0,
                 35.0, 25.0, 18.0, 13.0, 9.0, 6.5, 5.2],
})

for k, v in [("csv_activo", False), ("tajeo", DEFAULT_TAJEO.copy()),
             ("mezcla", DEFAULT_MEZCLA.copy()), ("barricada", DEFAULT_BARRICADA.copy()),
             ("ucs_df", DEFAULT_UCS.copy()), ("granu_df", DEFAULT_GRANU.copy())]:
...
<div class="hero-banner">
  ...
  <p>Diseño de mezcla · Resistencia UCS · Presión sobre barricada · Granulometría (ASTM D2487 / D6913 / D1140) · Reporte técnico</p>
</div>
""", unsafe_allow_html=True)
...
if uploaded_csv is not None:
    try:
        ...
        if "GRANULOMETRIA" in sections:
            dg = sections["GRANULOMETRIA"].copy()
            dg.columns = ["Tamiz_mm", "Pasa_pct"]
            dg["Tamiz_mm"] = pd.to_numeric(dg["Tamiz_mm"], errors="coerce")
            dg["Pasa_pct"] = pd.to_numeric(dg["Pasa_pct"], errors="coerce")
            st.session_state.granu_df = dg.dropna().reset_index(drop=True)
...
if st.session_state.csv_activo:
    if st.button("🔄 Restaurar datos por defecto"):
        for k in ["csv_activo", "tajeo", "mezcla", "barricada", "ucs_df", "granu_df"]:
...
with tabs[3]:
    ...
    g = 9.81
    P_hidrost = rho_b * g * h_b / 1000
    delta_P   = (2 * tau0 * h_b / R_t) / 1000
...
# ══════════════════════════════
with tabs[4]:
    st.markdown("## 🔬 Granulometría del Relave — ASTM D6913 / D2487 / D1140")

    st.markdown("""
    <div class="info-box">
    <b>Base normativa:</b> Los análisis granulométricos siguen los procedimientos de
    <b>ASTM D6913</b> (distribución de tamaño de partículas por tamizado),
    <b>ASTM D2487</b> (clasificación SUCS) y
    <b>ASTM D1140</b> (material fino por lavado — tamiz N°200).
    Los resultados se vinculan con la densidad del relave y el contenido de sólidos definidos en la pestaña de Dosificación.
    </div>
    """, unsafe_allow_html=True)

    # ── Recuperar datos de otras pestañas ──
    mezcla_g = st.session_state.mezcla
    rho_relave_g    = float(mezcla_g["densidad_relave_kgm3"])
    contenido_sol_g = float(mezcla_g["contenido_solidos_pct"])

    granu_df = st.session_state.granu_df.copy()

    # ── Sección: Ingreso / edición de datos granulométricos ──
    section_hdr("Datos Granulométricos — Tabla de Tamizado (ASTM D6913 Método A)")

    st.markdown("""
    <div class="info-box" style="margin-bottom:10px;">
    Ingrese los datos de la curva granulométrica del relave. Tamices normalizados según
    <b>ASTM D6913</b> (Tabla 1). El porcentaje que pasa el tamiz N°200 (0.075 mm) se considera
    como material fino por lavado según <b>ASTM D1140</b>.
    </div>
    """, unsafe_allow_html=True)

    col_edit, col_info_g = st.columns([2, 1])
    with col_edit:
        granu_edit = st.data_editor(
            granu_df[["Tamiz_mm", "Pasa_pct"]].rename(
                columns={"Tamiz_mm": "Tamiz (mm)", "Pasa_pct": "% Pasa"}),
            num_rows="dynamic",
            use_container_width=True,
            key="granu_editor"
        )
        if st.button("✔ Actualizar datos granulométricos"):
            new_df = granu_edit.rename(
                columns={"Tamiz (mm)": "Tamiz_mm", "% Pasa": "Pasa_pct"})
            new_df["Tamiz_mm"] = pd.to_numeric(new_df["Tamiz_mm"], errors="coerce")
            new_df["Pasa_pct"] = pd.to_numeric(new_df["Pasa_pct"], errors="coerce")
            st.session_state.granu_df = new_df.dropna().reset_index(drop=True)
            st.success("✔ Datos granulométricos actualizados.")
            st.rerun()

    with col_info_g:
        st.markdown("""
        <div class="formula-box">
        Tamices normalizados ASTM D6913:<br><br>
        3" = 75.0 mm<br>
        2" = 50.0 mm<br>
        1½" = 37.5 mm<br>
        1" = 25.0 mm<br>
    ...
    granu_calc = st.session_state.granu_df.copy()
    try:
        df_proc, pct_grava, pct_arena, pct_finos, D10, D30, D60, Cu, Cc, pasa_N4, pasa_N200 = \
            calcular_granulometria(granu_calc)

        # ── Clasificación SUCS ──
        sucs_sym, sucs_nom = clasificar_sucs(
            pct_grava, pct_arena, pct_finos, Cu or 0, Cc or 0)

        # ── Métricas principales ──
        section_hdr("Fracciones Granulométricas y Parámetros (ASTM D2487 / D6913)")
        m1g, m2g, m3g, m4g = st.columns(4)
        with m1g: st.markdown(metric_card("% Grava (>4.75 mm)", f"{pct_grava:.1f}", "%"), unsafe_allow_html=True)
        with m2g: st.markdown(metric_card("% Arena (0.075–4.75 mm)", f"{pct_arena:.1f}", "%"), unsafe_allow_html=True)
        with m3g: st.markdown(metric_card("% Finos (<0.075 mm)", f"{pct_finos:.1f}", "%"), unsafe_allow_html=True)
        with m4g: st.markdown(metric_card("Material pasa N°200 (lavado)", f"{pasa_N200:.1f}", "%"), unsafe_allow_html=True)

        m5g, m6g, m7g, m8g = st.columns(4)
        with m5g:
            st.markdown(metric_card("D₁₀ (mm)", f"{D10:.4f}" if D10 else "N/D", "mm"), unsafe_allow_html=True)
        with m6g:
            st.markdown(metric_card("D₃₀ (mm)", f"{D30:.4f}" if D30 else "N/D", "mm"), unsafe_allow_html=True)
        with m7g:
            st.markdown(metric_card("D₆₀ (mm)", f"{D60:.4f}" if D60 else "N/D", "mm"), unsafe_allow_html=True)
        with m8g:
            st.markdown(metric_card("Clasificación SUCS", sucs_sym, ""), unsafe_allow_html=True)

        # ── Tabla de tamizado completa ──
        section_hdr("Tabla Completa de Tamizado (ASTM D6913 — Método A)")
        ca_t, cb_t = st.columns(2)
        with ca_t:
            df_tabla = df_proc[["Etiqueta", "Tamiz_mm", "Pasa_pct", "Ret_acum_pct", "Ret_indiv_pct"]].copy()
            df_tabla.columns = ["Tamiz", "Abertura (mm)", "% Pasa", "% Ret. Acum.", "% Ret. Indiv."]
            df_tabla["% Pasa"]       = df_tabla["% Pasa"].map(lambda x: f"{x:.1f}")
            df_tabla["% Ret. Acum."] = df_tabla["% Ret. Acum."].map(lambda x: f"{x:.1f}")
            df_tabla["% Ret. Indiv."]= df_tabla["% Ret. Indiv."].map(lambda x: f"{x:.1f}")
            st.markdown(render_table(df_tabla), unsafe_allow_html=True)
        ...
        with cb_t:
            ...
            df_params = pd.DataFrame({
                "Parámetro": [
                    "D₁₀ (mm)", "D₃₀ (mm)", "D₆₀ (mm)",
                    "Cu = D₆₀/D₁₀", "Cc = D₃₀²/(D₁₀×D₆₀)",
                    "% Grava (>N°4)", "% Arena (N°4–N°200)",
                    "% Finos (<N°200)", "Pasa N°200 — lavado (D1140)",
                    "Clasificación SUCS (D2487)", "Nombre del grupo",
                ],
                "Valor": [
                    f"{D10:.4f}" if D10 else "N/D",
                    f"{D30:.4f}" if D30 else "N/D",
                    f"{D60:.4f}" if D60 else "N/D",
                    f"{Cu:.2f}" if Cu else "N/D",
                    f"{Cc:.2f}" if Cc else "N/D",
                    f"{pct_grava:.1f}%",
                    f"{pct_arena:.1f}%",
                    f"{pct_finos:.1f}%",
                    f"{pasa_N200:.1f}%",
            ...
            st.markdown(render_table(df_params), unsafe_allow_html=True)

            # Interpretación Cu y Cc
            if Cu and Cc:
                bien_gradada_arena = Cu >= 6 and 1 <= Cc <= 3
                bien_gradada_grava = Cu >= 4 and 1 <= Cc <= 3
                st.markdown(f"""
                <div class="formula-box" style="margin-top:10px;">
                Cu = D₆₀/D₁₀ = {D60:.4f}/{D10:.4f} = {Cu:.2f}<br>
                {'→ Cu ≥ 6 ✅ (arena bien gradada)' if Cu>=6 else '→ Cu < 6 (arena mal gradada)'}<br><br>
                Cc = D₃₀² / (D₁₀×D₆₀)<br>
                   = {D30:.4f}² / ({D10:.4f}×{D60:.4f})<br>
                   = {Cc:.3f}<br>
                {'→ 1 ≤ Cc ≤ 3 ✅ (bien gradada)' if 1<=Cc<=3 else '→ Cc fuera de rango (mal gradada)'}
                </div>
                """, unsafe_allow_html=True)

        # ── Curva Granulométrica ──
        section_hdr("Curva Granulométrica — Distribución de Tamaño de Partículas (ASTM D6913)")

        fig_granu = go.Figure()

        # Área bajo la curva
        fig_granu.add_trace(go.Scatter(
            x=df_proc["Tamiz_mm"].values,
            y=df_proc["Pasa_pct"].values,
            mode="lines+markers",
            fill="tozeroy",
            fillcolor="rgba(43,108,176,0.12)",
            line=dict(color="#2b6cb0", width=3),
            marker=dict(size=9, color="#1a365d",
                        line=dict(color="white", width=2)),
            name="Curva granulométrica",
            hovertemplate="<b>%{x:.3f} mm</b><br>Pasa: %{y:.1f}%<extra></extra>"
        ))

        # Líneas verticales de D10, D30, D60
        for Dx, label, color in [(D10, "D₁₀", "#e53e3e"),
                                  (D30, "D₃₀", "#dd6b20"),
                                  (D60, "D₆₀", "#276749")]:
            if Dx:
                fig_granu.add_vline(x=Dx, line_dash="dot", line_color=color, line_width=1.5,
                                    annotation_text=f"{label}={Dx:.3f}mm",
                                    annotation_font_color=color,
                                    annotation_font_size=11)

        # Líneas horizontales de referencia
        fig_granu.add_hline(y=10, line_dash="dot", line_color="#bee3f8", line_width=1)
        fig_granu.add_hline(y=30, line_dash="dot", line_color="#bee3f8", line_width=1)
        fig_granu.add_hline(y=60, line_dash="dot", line_color="#bee3f8", line_width=1)

        # Zonas de fracción
        fig_granu.add_vrect(x0=0.001, x1=0.075,
                            fillcolor="rgba(235,248,255,0.3)", line_width=0,
                            annotation_text="Finos", annotation_position="top left",
                            annotation_font_size=10, annotation_font_color="#2b6cb0")
        fig_granu.add_vrect(x0=0.075, x1=4.75,
                            fillcolor="rgba(255,250,235,0.3)", line_width=0,
                            annotation_text="Arena", annotation_position="top left",
                            annotation_font_size=10, annotation_font_color="#744210")
        fig_granu.add_vrect(x0=4.75, x1=100,
                            fillcolor="rgba(240,255,244,0.3)", line_width=0,
                            annotation_text="Grava", annotation_position="top left",
                            annotation_font_size=10, annotation_font_color="#276749")

        fig_granu.update_layout(
            **PLOTLY_LAYOUT,
            height=480,
            title=dict(text=f"Curva Granulométrica — {sucs_sym}: {sucs_nom}", font=dict(size=14))
            
        )
        st.plotly_chart(fig_granu, use_container_width=True)

        # ── Material fino por lavado (ASTM D1140) ──
        section_hdr("Análisis de Material Fino por Lavado — ASTM D1140")

        ca_l, cb_l = st.columns(2)
        with ca_l:
            # Simulación del cálculo ASTM D1140
            # A = [(B - C) / B] × 100
            # B = masa seca original, C = masa retenida en N°200 después del lavado
            # Usando porcentajes directamente de la curva
            masa_seca_ref = 500.0  # g (referencia típica para relave)
            masa_ret_N200 = masa_seca_ref * (1 - pasa_N200 / 100)
            masa_pasa_lavado = masa_seca_ref * (pasa_N200 / 100)
            pct_fino_lavado = ((masa_seca_ref - masa_ret_N200) / masa_seca_ref) * 100

            df_lavado = pd.DataFrame({
                "Parámetro ASTM D1140": [
                    "Masa seca inicial (B)",
                    "Masa retenida N°200 post-lavado (C)",
                    "Masa pasa N°200 por lavado",
                    "% material fino por lavado (A)",
                    "Norma aplicable",
                    "Método empleado",
                ],
                "Valor": [
                    f"{masa_seca_ref:.1f} g (referencia)",
                    f"{masa_ret_N200:.1f} g",
                    f"{masa_pasa_lavado:.1f} g",
                    f"{pct_fino_lavado:.1f}%",
                    "ASTM D1140-00",
                    "Método B (con dispersante)",
                ],
            })
            st.markdown(render_table(df_lavado), unsafe_allow_html=True)
            st.markdown(result_highlight(
                "% finos por lavado (ASTM D1140)",
                f"{pct_fino_lavado:.1f}%"
            ), unsafe_allow_html=True)
        ...
        with cb_l:
            st.markdown(f"""
            <div class="formula-box">
            ASTM D1140 — Fórmula de cálculo:<br><br>
            A = [(B − C) / B] × 100<br><br>
            Donde:<br>
            A = % material más fino que N°200 por lavado<br>
            B = masa seca original del espécimen (g)<br>
            C = masa seca retenida en N°200 después<br>
                del lavado (g)<br><br>
            Resultado:<br>
            A = [({masa_seca_ref:.0f} − {masa_ret_N200:.1f}) / {masa_seca_ref:.0f}] × 100<br>
            A = {pct_fino_lavado:.1f}%<br><br>
            Método B: inmersión previa con hexametafosfato<br>
            de sodio (40 g/1000 mL) — mínimo 2 horas
            </div>
            """, unsafe_allow_html=True)

        # ── Aptitud del relave para Paste Fill ──
        section_hdr("Evaluación de Aptitud del Relave para Relleno en Pasta")

        df_aptitud, n_cumple, apto = evaluar_granulometria_pasta(
            pct_finos, pasa_N200, rho_relave_g, contenido_sol_g)

        col_apt1, col_apt2 = st.columns([2, 1])
        with col_apt1:
            st.markdown(render_table(df_aptitud), unsafe_allow_html=True)

        with col_apt2:
            color_apto = "#276749" if apto else "#c53030"
            icono_apto = "✅" if apto else "❌"
            st.markdown(f"""
            <div class="result-highlight" style="text-align:center; padding:20px;">
                <div class="label">Veredicto de Aptitud</div>
                <div style="font-size:40px; margin:10px 0;">{icono_apto}</div>
                <div style="color:{color_apto}; font-size:18px; font-weight:700;">
                    {'APTO para Paste Fill' if apto else 'REVISAR diseño'}
                </div>
                <div style="margin-top:10px; color:#1a365d; font-size:13px;">
                    {n_cumple}/5 criterios cumplidos
                </div>
            </div>
            """, unsafe_allow_html=True)

        # ── Gráfico barras fracciones + D-D-D ──
        section_hdr("Distribución de Fracciones y Parámetros de Gradación")
        cg1, cg2 = st.columns(2)

        with cg1:
            fig_frac = go.Figure(go.Bar(
                x=["Grava\n(>4.75 mm)", "Arena\n(0.075–4.75 mm)", "Finos\n(<0.075 mm)"],
                y=[pct_grava, pct_arena, pct_finos],
                marker_color=["#276749", "#dd6b20", "#2b6cb0"],
                text=[f"{v:.1f}%" for v in [pct_grava, pct_arena, pct_finos]],
                textposition="outside",
                textfont=dict(size=13, color="#1a365d"),
            ))
            fig_frac.add_hline(y=15, line_dash="dash", line_color="#e53e3e",
                               annotation_text="Mín. finos PF = 15%",
                               annotation_font_color="#c53030")
            fig_frac.update_layout(**PLOTLY_LAYOUT, height=340,
                title=dict(text="Distribución de Fracciones — ASTM D2487", font=dict(size=14)),
                yaxis=dict(title="% en masa",
                    range=[0, 110],
                     gridcolor="#bee3f8",
                    linecolor="#90cdf4"
                )
        ...
        with cg2:
            # Gráfico Plasticity Chart conceptual / radar de cumplimiento
            categorias = ["% Finos ≥15%", "% Finos ≤70%", "Pasa N°200 ≥15%",
                          "Densidad\nrelave", "Contenido\nsólidos"]
            valores_norm = [
                min(pct_finos / 15, 2.0),
                min((70 - max(pct_finos - 70, 0)) / 70, 1.0) if pct_finos <= 70 else 0.2,
                min(pasa_N200 / 15, 2.0),
                1.0 if 2600 <= rho_relave_g <= 3200 else 0.3,
                1.0 if 70 <= contenido_sol_g <= 85 else 0.4,
            ]
            fig_radar = go.Figure(go.Scatterpolar(
                ...
                marker=dict(size=8, color="#1a365d"),
                name="Cumplimiento"
            ))
            fig_radar.add_trace(go.Scatterpolar(
                r=[1.0] * len(categorias),
                theta=categorias,
            ...
            fig_radar.update_layout(
                polar=dict(
                    radialaxis=dict(visible=True, range=[0, 2],
                                   ...
                                   gridcolor="#bee3f8"),
                    angularaxis=dict(gridcolor="#bee3f8")
                ),
                paper_bgcolor="#ffffff",
                plot_bgcolor="#f8fbff",
                font=dict(family="IBM Plex Sans, sans-serif", color="#1a1a2e", size=11),
                legend=dict(font=dict(size=11), bgcolor="rgba(235,248,255,0.95)"),
                margin=dict(l=40, r=40, t=60, b=40),
                height=340,
                title=dict(text="Radar de Aptitud para Paste Fill", font=dict(size=14))
            )
            st.plotly_chart(fig_radar, use_container_width=True)

        # ── Resumen clasificación SUCS ──
        section_hdr(f"Clasificación SUCS del Relave — ASTM D2487: {sucs_sym} — {sucs_nom}")
        st.markdown(f"""
        <div class="info-box">
        <b>Símbolo de grupo:</b> {sucs_sym} — <b>{sucs_nom}</b><br><br>
        <b>Criterios aplicados (ASTM D2487):</b><br>
        • Material > tamiz N°200 (75 µm): {100-pct_finos:.1f}% → Suelo {'fino' if pct_finos>=50 else 'grueso'}<br>
        • % Grava (retenido N°4): {pct_grava:.1f}% — % Arena (pasa N°4, ret. N°200): {pct_arena:.1f}%<br>
        • % Finos (pasa N°200): {pct_finos:.1f}%<br>
        • Cu = {f'{Cu:.2f}' if Cu else 'N/D'} — Cc = {f'{Cc:.2f}' if Cc else 'N/D'}<br><br>
        <b>Referencia normativa:</b> ASTM D2487-17 — Sistema Unificado de Clasificación de Suelos (SUCS).<br>
        La clasificación se basa en la distribución granulométrica por tamizado (ASTM D6913)
        y el material fino por lavado (ASTM D1140).
        </div>
        """, unsafe_allow_html=True)

        # Guardar resultados en session_state para uso en Reporte Final
        st.session_state["granu_resultados"] = {
            "pct_grava": pct_grava,
            "pct_arena": pct_arena,
            "pct_finos": pct_finos,
            "pasa_N200": pasa_N200,
            ...
            "sucs_sym": sucs_sym,
            "sucs_nom": sucs_nom,
            "pct_fino_lavado": pct_fino_lavado,
            "apto": apto,
            "n_cumple": n_cumple,
            "df_aptitud": df_aptitud,
        }

    except Exception as e:
        st.error(f"❌ Error en cálculo granulométrico: {e}. Verifique que los datos tengan columnas 'Tamiz_mm' y 'Pasa_pct' válidas.")
...
with tabs[5]:
    ...
    # Recuperar datos granulométricos
    gr = st.session_state.get("granu_resultados", None)
    ...
    # Indicadores granulométricos en el resumen
    if gr:
        section_hdr("Indicadores Clave — Granulometría del Relave (ASTM D2487 / D6913 / D1140)")
        rg1, rg2, rg3, rg4 = st.columns(4)
        with rg1: st.markdown(metric_card("% Finos (<N°200)", f"{gr['pct_finos']:.1f}", "%"), unsafe_allow_html=True)
        with rg2: st.markdown(metric_card("% Pasa N°200 (lavado)", f"{gr['pasa_N200']:.1f}", "%"), unsafe_allow_html=True)
        with rg3: st.markdown(metric_card("Clasificación SUCS", gr["sucs_sym"], ""), unsafe_allow_html=True)
        with rg4:
            ico_g = "✅" if gr["apto"] else "❌"
            st.markdown(metric_card(f"{ico_g} Aptitud Paste Fill",
                                    f"{gr['n_cumple']}/5", "criterios"), unsafe_allow_html=True)

        rg5, rg6, rg7, rg8 = st.columns(4)
        with rg5: st.markdown(metric_card("% Grava (>N°4)", f"{gr['pct_grava']:.1f}", "%"), unsafe_allow_html=True)
        with rg6: st.markdown(metric_card("% Arena (N°4–N°200)", f"{gr['pct_arena']:.1f}", "%"), unsafe_allow_html=True)
        with rg7:
            st.markdown(metric_card("Cu", f"{gr['Cu']:.2f}" if gr['Cu'] else "N/D", "—"), unsafe_allow_html=True)
        with rg8:
            st.markdown(metric_card("Cc", f"{gr['Cc']:.2f}" if gr['Cc'] else "N/D", "—"), unsafe_allow_html=True)

    section_hdr("📄 Exportar Reporte PDF")
    st.markdown("""
    <div class="info-box">
    Genera un reporte PDF completo con todos los parámetros, tablas de resultados
    y gráficos del diseño, incluyendo: distribución de presión, dosificación, UCS vs días,
    comparativa 28 días, curva granulométrica, fracciones y aptitud para Paste Fill.
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("📥 Generar Reporte PDF — Relleno en Pasta", use_container_width=True):
        ...
        # Gráfico 4 — Curva granulométrica para PDF
        fig_p4 = None
        fig_p5 = None
        if gr:
            try:
                granu_pdf = st.session_state.granu_df.copy()
                df_proc_pdf, _, _, _, _, _, _, _, _, _, _ = calcular_granulometria(granu_pdf)
                fig_p4 = go.Figure()
                fig_p4.add_trace(go.Scatter(
                    x=df_proc_pdf["Tamiz_mm"].values,
                    y=df_proc_pdf["Pasa_pct"].values,
                ...
                for Dx, lbl, col_ in [(gr["D10"], "D₁₀", "#e53e3e"),
                                       (gr["D30"], "D₃₀", "#dd6b20"),
                                       (gr["D60"], "D₆₀", "#276749")]:
                    if Dx:
                        fig_p4.add_vline(x=Dx, line_dash="dot", line_color=col_, line_width=1.5,
                                         annotation_text=f"{lbl}={Dx:.3f}mm",
                                         annotation_font_color=col_, annotation_font_size=10)
                fig_p4.update_layout(**LP_pdf, height=500,
                    title=dict(text=f"Curva Granulométrica — {gr['sucs_sym']}: {gr['sucs_nom']}", font=dict(size=15)),
                    xaxis=dict(type="log", title="Tamaño de partícula (mm)",
                               tickvals=[0.075, 0.425, 2.0, 4.75, 19.0, 75.0],
                               ticktext=["N°200\n0.075", "N°40\n0.425", "N°10\n2.0",
                                         "N°4\n4.75", "¾\"\n19", "3\"\n75"],
                               gridcolor="#bee3f8", autorange="reversed"),
                    yaxis=dict(title="% que pasa", range=[0, 105], gridcolor="#bee3f8"))

                # Gráfico 5 — Fracciones
                fig_p5 = go.Figure(go.Bar(
                    x=["Grava (>4.75mm)", "Arena (0.075–4.75mm)", "Finos (<0.075mm)"],
                    y=[gr["pct_grava"], gr["pct_arena"], gr["pct_finos"]],
                    marker_color=["#276749", "#dd6b20", "#2b6cb0"],
                    text=[f"{v:.1f}%" for v in [gr["pct_grava"], gr["pct_arena"], gr["pct_finos"]]],
                    textposition="outside",
                ))
                fig_p5.add_hline(y=15, line_dash="dash", line_color="#e53e3e",
                                 annotation_text="Mín. finos PF=15%", annotation_font_color="#c53030")
                fig_p5.update_layout(**LP_pdf, height=500,
                    title=dict(text="Distribución de Fracciones — ASTM D2487", font=dict(size=15)),
                    yaxis=dict(title="% en masa", range=[0, 115], gridcolor="#bee3f8"),
                    xaxis=dict(gridcolor="#bee3f8"))
            except Exception:
                pass
        ...
        story.append(Paragraph("Reporte Tecnico — Diseno de Mezcla, UCS, Barricada y Granulometria", S_SUB))
        story.append(hr())
        ...
        data4 = [["Parametro", "Valor"],
                 ...
                 ["Limite elastico tau0 (Pa)",  f"{tau02:.0f}"],
                 ["Radio tuberia R (m)",         f"{R_t2:.1f}"],
                 ["Delta_P limite elastico (kPa)", f"{dP2:.3f}"],
                 ["PRESION TOTAL (kPa)",           f"{P_tot2:.2f}"],
                 ["Peso barricada (kN)",           f"{peso_b2:.1f}"],
                 ]
        story.append(make_tbl(data4, col_w=[9.5 * cm, 6.5 * cm]))

        # Sección 5 — Granulometría
        if gr:
            story.append(Spacer(1, 0.4 * cm))
            story.append(Paragraph("5. Granulometria del Relave — ASTM D6913 / D2487 / D1140", S_H2))
            story.append(hr())
            data5 = [["Parametro", "Valor"],
                     ["% Grava (> N4 / 4.75 mm)", f"{gr['pct_grava']:.1f}%"],
                     ["% Arena (N4 a N200)", f"{gr['pct_arena']:.1f}%"],
                     ["% Finos (< N200 / 0.075 mm)", f"{gr['pct_finos']:.1f}%"],
                     ["% Pasa N200 por lavado (ASTM D1140)", f"{gr['pasa_N200']:.1f}%"],
                     ["D10 (mm)", f"{gr['D10']:.4f}" if gr['D10'] else "N/D"],
                     ["D30 (mm)", f"{gr['D30']:.4f}" if gr['D30'] else "N/D"],
                     ["D60 (mm)", f"{gr['D60']:.4f}" if gr['D60'] else "N/D"],
                     ["Cu = D60/D10", f"{gr['Cu']:.2f}" if gr['Cu'] else "N/D"],
                     ["Cc = D30^2/(D10*D60)", f"{gr['Cc']:.2f}" if gr['Cc'] else "N/D"],
                     ["Clasificacion SUCS (D2487)", gr['sucs_sym']],
                     ["Nombre del grupo", gr['sucs_nom']],
                     ["Aptitud para Paste Fill", "APTO" if gr['apto'] else "REVISAR"],
                     ["Criterios cumplidos", f"{gr['n_cumple']}/5"],
                     ]
            story.append(make_tbl(data5, col_w=[9.5 * cm, 6.5 * cm]))
            story.append(Spacer(1, 0.2 * cm))

            # Tabla de aptitud
            story.append(Paragraph("5.1  Tabla de criterios de aptitud para Paste Fill:", S_BODY))
            story.append(Spacer(1, 0.15 * cm))
            apt_rows = [["Criterio", "Valor", "Limite", "Estado"]]
            for _, row_apt in gr["df_aptitud"].iterrows():
                apt_rows.append([
                    row_apt["Criterio"], row_apt["Valor"],
                    row_apt["Límite"], row_apt["Estado"]
                ])
            story.append(make_tbl(apt_rows,
                                   col_w=[7.5 * cm, 2.5 * cm, 2.5 * cm, 3.5 * cm]))

            # Sección 6 — Gráficos (incluye granulometría)
            story.append(Spacer(1, 0.5 * cm))
            story.append(Paragraph("6. Graficos Tecnicos", S_H2))
        else:
            story.append(Spacer(1, 0.5 * cm))
        ...
        story.append(hr())
        try:
            story.append(Paragraph("Distribucion de Presion Vertical en el Tajeo", S_BODY))
            story.append(Spacer(1, 0.2 * cm))
            story.append(fig_to_rli(fig_p1))
            story.append(Spacer(1, 0.5 * cm))
            story.append(Paragraph("Curva de Resistencia UCS vs Dias de Curado", S_BODY))
            story.append(Spacer(1, 0.2 * cm))
            story.append(fig_to_rli(fig_p2))
            story.append(Spacer(1, 0.5 * cm))
            story.append(Paragraph("Comparativa UCS 28 dias vs Requerimiento", S_BODY))
            story.append(Spacer(1, 0.2 * cm))
            story.append(fig_to_rli(fig_p3))
            if fig_p4:
                story.append(Spacer(1, 0.5 * cm))
                story.append(Paragraph(
                    f"Curva Granulometrica del Relave — ASTM D6913 ({gr['sucs_sym']}: {gr['sucs_nom']})",
                    S_BODY))
                story.append(Spacer(1, 0.2 * cm))
                story.append(fig_to_rli(fig_p4))
            if fig_p5:
                story.append(Spacer(1, 0.5 * cm))
                story.append(Paragraph(
                    "Distribucion de Fracciones Granulometricas — ASTM D2487", S_BODY))
                story.append(Spacer(1, 0.2 * cm))
                story.append(fig_to_rli(fig_p5))
        ...
        story.append(hr())
        story.append(Paragraph(
            f"Sistema PF v1.1  |  Generado: {hoy_str}  |  Normas: ASTM D2487 / D6913 / D1140 / Mitchell 1982",
            S_RIGHT))
...
<div class="footer-box">
    ...
    <div>Diseño de mezcla · Resistencia UCS · Presión sobre barricada · Granulometría (ASTM D2487 / D6913 / D1140)</div>
    <br>
    <div>Versión 1.1 · {hoy} · Landriault et al., 1996 / Mitchell, 1982 / ASTM International</div>
</div>
""", unsafe_allow_html=True)
